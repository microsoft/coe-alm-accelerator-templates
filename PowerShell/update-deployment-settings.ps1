function Set-DeploymentSettingsConfiguration
{
    ...
    Write-Information -MessageData (ConvertTo-Json -Depth 10 $configurationData)


    #Generate Deployment Settings
    Write-Information -MessageData "Update Deployment Settings"
    ...
    foreach($configurationDataEnvironment in $configurationData)
    {
        ...
        Write-Information -MessageData $buildDefinitionResourceUrl
        ...
        Write-Information -MessageData "Retrieved " $buildDefinitionResponseResults.length " builds"
        ...
        if($null -ne $configurationDataEnvironment -and $null -ne $configurationDataEnvironment.UserSettings) {
            ...
            if(Test-Path "$buildSourceDirectory\$repo\$solutionName\config\$environmentName\") {
                Write-Information -MessageData "Deleting $buildSourceDirectory\$repo\$solutionName\config\$environmentName\*eploymentSettings.json"
                Remove-Item -Path "$buildSourceDirectory\$repo\$solutionName\config\$environmentName\*eploymentSettings.json" -Force
            }
            else {
                Write-Information -MessageData "Creating $buildSourceDirectory\$repo\$solutionName\config" -Name "$environmentName" -ItemType "directory"
            ...
            Write-Information -MessageData "Creating deployment settings"
            ...
            Write-Information -MessageData "Creating custom deployment settings"
...
function New-DeploymentPipeline
{
    ...
    if($null -ne $configurationData -and $configurationData.length -gt 0) {
        Write-Information -MessageData "Retrieved " $configurationData.length " deployment environments"
        $branchResourceUrl = "$orgUrl$projectName/_apis/git/repositories/$repo/refs?filter=heads/$solutionName&api-version=6.0"
        Write-Information -MessageData $branchResourceUrl
        ...
        Write-Information -MessageData "Retrieved " $branchResourceResults.length " branch"
        ...
        Write-Information -MessageData "Retrieved " $deploymentConfigurationData.length " deployment configurations"
        ...
        if($branchResourceResults.length -eq 0 -or $buildDefinitionResponseResults.length -lt $deploymentConfigurationData.length) {
            ...
            if(-Not [string]::IsNullOrWhiteSpace($settings)) {
                ...
                Write-Information -MessageData "environments: " $settings
                ...
                if(Test-Path ".\combined.log") {
                    Write-Information -MessageData ((Get-Content ".\combined.log") -join "`n")
                }
...
function Create-Update-ServiceConnection-Parameter{
    ...
    Write-Information -MessageData "Inside Create-Update-ServiceConnection-Parameter"
    Write-Information -MessageData "newBuildDefinitionVariables - $newBuildDefinitionVariables"
     if($null -ne $newBuildDefinitionVariables){
